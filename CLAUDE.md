...
- Root `composer.json` uses `@dev` constraints for all aurora/* packages
...
- `bin/aurora` — CLI entry point (SQLite + file config)

## Code Style
- PHP 8.3+, `declare(strict_types=1)` in every file
- Namespace pattern: `Aurora\PackageName\` (e.g., `Aurora\Entity\`, `Aurora\AI\Schema\`)
- Test namespace: `Aurora\PackageName\Tests\Unit\` or `Aurora\Tests\Integration\PhaseN\`
...
- **InMemoryEntityStorage** (`Aurora\Api\Tests\Fixtures\`) — use for tests. SqlEntityStorage for real storage.
...
## Environment
- `AURORA_DB` — SQLite database path (default: `./aurora.sqlite`)
- `AURORA_CONFIG_DIR` — config sync directory (default: `./config/sync`)
