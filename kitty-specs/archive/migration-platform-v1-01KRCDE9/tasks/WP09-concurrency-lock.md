...
- 2026-05-13T16:23:14Z – claude:opus:waaseyaa-reviewer:reviewer – shell_pid=17095 – Approved. Per-migration filesystem advisory lock via flock(LOCK_EX|LOCK_NB) against <cwd>/storage/migration-locks/<id>.lock. MigrationLock writes getmypid() into the body for operator visibility; release() is idempotent (LOCK_UN + fclose + @unlink). pcntl_signal(SIGTERM/SIGINT) handler installed when pcntl available, falling back to register_shutdown_function on Windows / no-pcntl SAPIs. Signal handler exits 128+signal. All 5 mutating import:* commands gated: ImportRun, ImportRunAll (per-migration in loop, not global), ImportResume, ImportRollback (--confirm only; preview path is read-only no-lock), ImportReset (--confirm only). import:status untouched. MigrationConcurrencyException caught -> exit 3 per contracts/cli-runner.md (divergence from prompt body documented). registry->has() runs BEFORE acquire; smoke verified: bin/waaseyaa import:run nonexistent -> exit 2, storage/migration-locks/ never created. try/finally release in all 5 commands. No force-unlock CLI (operator rm per spec sec 9.3 D11; exception message embeds rm <lockPath> guidance). FR-061 + FR-062 covered by 4-scenario integration test (pcntl_fork contention with parent PID, proc_open release roundtrip, shutdown-function release on exit(1), Windows degradation). Gates: phpunit 8223/8223 OK; cs-check, phpstan, check-composer-policy, check-package-layers all clean; audit-dead-code findings are pre-existing baseline noise. Forward to WP11 e2e for two-process subprocess test.
