...
- 2026-05-21T01:39:06Z – claude:opus-4-7:reviewer:reviewer – shell_pid=829845 – WP05 wrap-up: CLAUDE.md schedule-entries checklist (7 steps, ScheduleEntriesInterface + manifest discovery + disabled_entries), ops playbook Schedule Entry Auto-Discovery section (built-in table + YAML opt-out + AgentScheduleEntries warning), CHANGELOG Unreleased Added(4)+Fixed(1) refs #1512/#1536, ScheduleListHandler grouped output by owning *ScheduleEntries FQCN + [disabled] markers + flat fallback (FR-008), 3 new tests (grouped/disabled/flat). composer cs-check + phpstan clean; 2453 phpunit tests pass.
