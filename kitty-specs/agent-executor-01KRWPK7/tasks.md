...
|---|---|---|---|
| T001 | External-consumer verification grep (`bin/check-external-consumers ai-agent-orphans`) | WP01 | | [D] |
| T002 | Scaffold `packages/ai-tools` (composer.json, ServiceProvider, autoload) | WP01 | | [D] |
| T003 | `AgentTool` VO + `AgentToolInterface` + `AgentToolResult` + `AbstractAgentTool` + `#[AsAgentTool]` | WP01 | | [D] |
| T004 | `AttributeToolRegistry` + `PackageManifestCompiler` discovery | WP01 | | [D] |
| T005 | Eight stock tools (`EntityRead/List/Create/Update/Delete/Search`, `RelationshipTraverse`, `VectorSearch`) | WP01 | [D] |
| T006 | Delete `packages/mcp/src/Tools/*` and rewire `McpController` against the new registry | WP01 | | [D] |
| T007 | Update `ai-schema/Mcp/*` generators; remove `McpToolDefinition` | WP01 | | [D] |
| T008 | Remove `McpToolExecutor::accessCheck(false)`; file ADR `0XX-mcp-tool-access-enforcement.md` | WP01 | | [D] |
| T009 | Gates: composer-policy, layers, dead-code, bulk-edit `occurrence_map.yaml` diff-compliance | WP01 | | [D] |
| T010 | `AgentRun` entity + entity type + enums (`RunStatus`, `HitlMode`, `EventType`) | WP02 | | [D] |
| T011 | `AgentAuditLog` entity + entity type | WP02 | | [D] |
| T012 | Migration `2026_05_18_000001_create_agent_run.php` with indexes | WP02 | | [D] |
| T013 | `AgentRunRepository` + `AgentAuditLogRepository` | WP02 | | [D] |
| T014 | `AgentRunAccessPolicy` (initiator ownership + bypass capability) | WP02 | | [D] |
| T015 | Capability seed updates (`agent.run`, `agent.run.approve`, `agent.run.bypass_ownership`, `tool.*`) | WP02 | | [D] |
| T016 | `AiAgentEntityServiceProvider` wiring + `composer.json` bumps | WP02 | | [D] |
| T017 | `AgentDefinition` VO + `#[AsAgentDefinition]` + `AgentDefinitionRegistry` | WP03 | | [D] |
| T018 | Extend `AgentResult` with `tokenUsageIn/Out`, `costCents` | WP03 | | [D] |
| T019 | Rewire `AgentExecutor` against new `ToolRegistry::register(AgentTool)` | WP03 | | [D] |
| T020 | HITL state machine (`none` / `all` / `interactive`) in `AgentExecutor` | WP03 | | [D] |
| T021 | Per-iteration + per-tool-call cancellation polling | WP03 | | [D] |
| T022 | Provider retry logic (3 retries, exp backoff cap 30 s) | WP03 | | [D] |
| T023 | Delete `AgentInterface`; sweep remaining references | WP03 | | [D] |
| T024 | `RunAgent` Messenger message + serialization | WP04 | | [D] |
| T025 | `RunAgentHandler` (status state machine, worker-concurrency guard) | WP04 | | [D] |
| T026 | `AgentRunService::enqueue()` + `runInline()` | WP04 | | [D] |
| T027 | `StalledRunReaper` service | WP04 | | [D] |
| T028 | Messenger transport wiring (`sync` for tests, real for prod) + provider/scalar config entities | WP04 | | [D] |
| T029 | `AgentRunController` (POST `/run`, GET, DELETE, POST `.../approve`) | WP05 | | [D] |
| T030 | `AgentRouteServiceProvider` in `packages/routing` | WP05 | | [D] |
| T031 | `AgentRunBroadcaster`: `BroadcastStorage::push` per SSE event vocabulary | WP05 | | [D] |
| T032 | Per-route capability checks + initiator-ownership enforcement | WP05 | | [D] |
| T033 | Request validator: bundle vs `agent_id`, destructive_approval normalization | WP05 | | [D] |
| T034 | `AiRunCommand` (`ai:run` with `--inline`, `--agent`, `--dry-run`, `--watch`, `--destructive-approval`) | WP06 | | [D] |
| T035 | `AiPurgeRunsCommand` (`ai:purge-runs`) | WP06 | [D] |
| T036 | `AiReapStalledRunsCommand` (`ai:reap-stalled-runs`) | WP06 | [D] |
| T037 | Scheduler entries: daily purge 03:00 UTC + 5-minute reaper | WP06 | | [D] |
| T038 | Streamable-HTTP MCP client implementation | WP07 | | [D] |
| T039 | `config.ai.mcp_servers` config entity wiring | WP07 | | [D] |
| T040 | Capability-prefix mapping + tool registration via `McpClientToolSource` | WP07 | | [D] |
| T041 | Stub-server integration test | WP07 | | [D] |
| T042 | `ModelPriceTable` (static per-provider/per-model pricing) | WP08 | [D] |
| T043 | `AgentRunTelemetryListener` (subscribes to AgentRun lifecycle events) | WP08 | | [D] |
| T044 | Token/cost/tool-count/latency capture + Telescope persistence | WP08 | | [D] |
...
**Subtasks:**
- [x] T001 External-consumer verification grep (WP01)
- [x] T002 Scaffold `packages/ai-tools` (WP01)
- [x] T003 `AgentTool` VO + interfaces + `#[AsAgentTool]` (WP01)
- [x] T004 `AttributeToolRegistry` + manifest-compiler discovery (WP01)
- [x] T005 Eight stock tool implementations (WP01)
- [x] T006 Delete `packages/mcp/src/Tools/*` and rewire `McpController` (WP01)
- [x] T007 Update `ai-schema/Mcp/*` generators; remove `McpToolDefinition` (WP01)
- [x] T008 Remove `accessCheck(false)`; file ADR (WP01)
- [x] T009 Gate verification + bulk-edit diff-compliance (WP01)
