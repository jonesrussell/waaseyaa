...
| FR-031 | Route registration for `/api/ai/agent/run*` SHALL live in `packages/routing` as `AgentRouteServiceProvider`, mirroring the `AuthOidcRouteServiceProvider` pattern (Layer-4 owns route wiring; Layer-5 owns controller logic). | Active |
