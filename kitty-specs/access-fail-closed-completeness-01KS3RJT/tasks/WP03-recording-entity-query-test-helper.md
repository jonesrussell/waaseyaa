...
created_at: '2026-05-20T23:56:27.433956+00:00'
subtasks:
- T012
- T013
- T014
...
- 2026-05-21T00:17:32Z – claude:opus-4-7:reviewer:reviewer – shell_pid=685994 – Review passed: helper extracted to packages/entity/testing/RecordingEntityQuery.php, autoload-dev only in both packages/entity/composer.json and root composer.json (verified composer dump-autoload --no-dev produces 0 RecordingEntityQuery entries in autoload_classmap/autoload_psr4/autoload_static). All 9 EntityQueryInterface methods implemented (chainable returns + execute() returns stubbedResults; accessChecks list<bool> records in order; boundAccount nullable). 711/711 phpunit tests pass on packages/entity+user+seo. composer phpstan (project-wide): 0 errors. composer cs-check (project-wide): 0 violations. bin/check-composer-policy: OK. Spot-check confirms unmigrated stubs at AuthControllerTest:88 (callCount field-name branching) and :196 (capturing-conditions list) have bespoke logic RecordingEntityQuery cannot serve — implementer judgment correct. Sibling commit 0188df256 is transparently labeled as WP01 deliverable (shared lane-c worktree, expected per topology).
