...
- 2026-05-21T00:25:08Z – claude:opus-4-7:reviewer:reviewer – shell_pid=703327 – Review passed: 4 retro *BindingTest.php files all pass and use RecordingEntityQuery from WP03; production patterns match test assertions (accessCheck(false) in all 4 callsites verified via grep); mutation test on PathAliasResolver confirmed regression guard catches binding removal. M-B.1 issue #1539 is open with correct title, baseline reference, and SearchController anonymous-path known-limitation documented. docs/specs/access-control.md has PolicyDependencyResolverInterface contract + 5-rule cascade + CI gate + Known limitations subsection. CHANGELOG [Unreleased] covers all five WPs. symfony-imports failure is pre-existing on main (11 violations unchanged). Scope tight.
